            ...
            <el-form-item required="true">
              ...
              <el-radio-group v-model="form.testmodels" @change="agreeChange">
                <el-tooltip class="box-item" effect="dark" placement="top-start">
                  <template #content>固定模板A<br/><br/>建立连接时间（0.06），首包时间（0.28），SSL握手时间（0.10）<br/><br/>网络延时（0.08），传输总时间（0.33），DNS解析时间（0.14）<br/><br/>内容下载速率（0.143），最大内容绘制速率LCP（0.714），缓冲时间（0.143）<br/><br/>传输丢包率（0.67），网络抖动（0.33），HTTP状态码</template>
                  <el-radio label="A" border>固定模板A</el-radio>
                </el-tooltip>
                <el-tooltip class="box-item" effect="dark" placement="top-start">
                  <template #content>固定模板B<br/><br/>建立连接时间（0.06），首包时间（0.28），SSL握手时间（0.10）<br/><br/>网络延时（0.08），传输总时间（0.33），DNS解析时间（0.14）<br/><br/>内容下载速率（0.143），最大内容绘制速率LCP（0.714），缓冲时间（0.143），HTTP状态码</template>
                  <el-radio label="B" border>固定模板B</el-radio>
                </el-tooltip>
                <el-tooltip class="box-item" effect="dark" placement="top-start">
                  <template #content>固定模板C<br/><br/>建立连接时间（0.06），首包时间（0.28），SSL握手时间（0.10）<br/><br/>网络延时（0.08），传输总时间（0.33），DNS解析时间（0.14）<br/><br/>传输丢包率（0.67），网络抖动（0.33），HTTP状态码</template>
        ...
        <el-card>
          <div class="mode09">
            ...
            <div class="webscreen">
             ...
             <!-- <img src="../../static/img/webscreentest.png"><iframe src="https://www.baidu.com/"></iframe>-->
            </div>
            <div class="showscore">
            ...
            <div class="download">
              <!--<div class="load">
              <el-button type="primary"><h4>再次测试</h4></el-button>
              <el-button style="margin-left:0;margin-top: 10px;"  type="primary"><h4>下载结果</h4></el-button>
              </div>-->
          ...
          <el-card>
            ...
            <div class="tableresultposition">
              <el-table class="datatable" :data="tableData" :header-cell-style="{'font-size':'18px', color:'#fff'}" :cell-style="{'font-size':'20px','font-weight':'600',fontFamily: 'Hiragino Sans GB'}" style="width: 100%">
                <el-table-column prop="http_code" label="状态码" fit="true">
                  <template slot-scope="scope">{{scope.row.http_code?scope.row.http_code:'/'}}</template>
                </el-table-column>
              ...
              <el-table class="datatable"  :data="tableData" :header-cell-style="{'font-size':'18px', color:'#fff'}" :cell-style="{'font-size':'20px','font-weight':'600',fontFamily: 'Hiragino Sans GB'}" style="width: 100%">
                ...
                <!--<el-table-column prop="onload_time" label="" fit="true">
                </el-table-column>
...
<script>
import {webwithoutcdntest} from '../api/api.js'
// const sleep = (delay) => new Promise((resolve) => setTimeout(resolve, delay))
const testOptions = ['http_code', 'connect_time', 'start_time', 'ssl_time', 'delay', 'total_time', 'dns_time', 'speed_download', 'lcp', 'fcp', 'packet_loss_rate', 'delay_jitter', 'web_size']
export default {
  ...
  data () {
    return {
      ...
      option1: {
        ...
        yAxis: {
          data: ['建立连接时间', '首包时间', 'SSL握手时间', '网络延时', '传输总时间', 'LCP', '缓冲时间', 'DNS解析时间', '内容下载速率', '传输丢包率', '网络抖动'],
          type: 'category',
          inverse: true,
          axisTick: {show: false},
          axisLine: {show: false},
      ...
      option3: {
        xAxis: {
          ...
          data: ['建立连接时间', '首包时间', 'SSL握手时间', '网络延时', '传输总时间', '最大内容绘制LCP', '缓冲时间', 'DNS解析时间', '内容下载速率', '传输丢包率', '网络抖动']
        },
        yAxis: {
          type: 'value'
        },
  ...
  methods: {
    ...
    organizedata (testdata) {
      ...
      let scoredata = []
      scoredata.push(testdata.connect_time_score, testdata.start_time_score, testdata.ssl_time_score, testdata.delay_score, testdata.total_time_score, testdata.lcp_score, testdata.fcp_score, testdata.dns_time_score, testdata.speed_download_score, testdata.packet_loss_rate_score, testdata.delay_jitter_score)
    ...
    onSubmit () {
      ...
      webwithoutcdntest(this.form).then(res => {
        ...
        this.$alert('测试失败，URL不可访问，请检查当前URL是否正确')
    ...
    agreeChange: function (val) {
      switch (val) {
        ...
        case 'B':
          this.disabled = true
          this.form.fixed = ['http_code', 'connect_time', 'start_time', 'ssl_time', 'delay', 'total_time', 'dns_time', 'speed_download', 'lcp', 'fcp', 'web_size']
          break
        case 'C':
          this.disabled = true
          this.form.fixed = ['http_code', 'connect_time', 'start_time', 'ssl_time', 'delay', 'total_time', 'dns_time', 'packet_loss_rate', 'delay_jitter', 'web_size']
          break
        case 'D':
          this.disabled = false
          this.form.fixed = []
